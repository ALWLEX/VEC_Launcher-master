...
public sealed class ModpackService
{
        ...
        await Parallel.ForEachAsync(needed,
            ...
            async (file, token) =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Log.Warn($"ModpackService: CurseForge file {file.ProjectId}/{file.FileId}: {ex.Message}");
                }
            }).ConfigureAwait(false);
        ...
        long projectId, long fileId, string modsDir, CancellationToken ct)
    {
        ...
        dlReq.Headers.TryAddWithoutValidation("User-Agent", "VEC Launcher/1.0");
    ...
    private static void ExtractOverrides(ZipArchive zip, string target, CancellationToken ct)
    {
        foreach (var entry in zip.Entries)
        {
            ...
            catch (Exception ex) { Log.Warn($"ModpackService: overrides {rel}: {ex.Message}"); }
        }
    }
        ...
        string mrpackPath, GameInstance instance, CancellationToken ct = default)
    {
        ...
        await Parallel.ForEachAsync(needed,
            ...
            async (file, token) =>
            {
                ...
                {
                    Log.Warn($"ModpackService: failed to download {file.Path}: {ex.Message}");
                }
            }).ConfigureAwait(false);
