...
public sealed class ModService
{
    ...
    private async Task<JsonDocument?> CurseGetAsync(string relativeUrl, CancellationToken ct)
    {
        ...
        var userKey = SettingsService.Load().CurseForgeApiKey;
        var keyToUse = !string.IsNullOrWhiteSpace(userKey) ? userKey : CurseForgeApiKey;

        using var req = new HttpRequestMessage(HttpMethod.Get, url);
        req.Headers.TryAddWithoutValidation("x-api-key", keyToUse);
        ...
        if (resp.StatusCode == HttpStatusCode.Forbidden || resp.StatusCode == HttpStatusCode.Unauthorized)
        {
            ...
            CurseForgeError = "CurseForge API недоступен без валидного ключа. Работа через Modrinth.";
            Log.Info("ModService: CurseForge API requires valid key (403). Switching to Modrinth.");
            return null;
        }
        ...
        {
            Log.Warn($"ModService: CurseForge returned {(int)resp.StatusCode} on {url}");
            return null;
        }
        ...
        ModContentType type, int limit, int offset, CancellationToken ct)
    {
        ...
        try
        {
            ...
            using var req = new HttpRequestMessage(HttpMethod.Get, url);
            req.Headers.TryAddWithoutValidation("User-Agent", "VEC Launcher/1.0 (Minecraft launcher)");

            using var resp = await _http.SendAsync(req, ct).ConfigureAwait(false);
            if (!resp.IsSuccessStatusCode)
            {
                Log.Warn($"ModService: Modrinth returned {(int)resp.StatusCode}");
                return (list, 0);
            }
        ...
        {
            Log.Warn($"ModService: Modrinth search error: {ex.Message}");
        }
        ...
        ModContentType type, int limit, int offset, CancellationToken ct)
    {
        ...
        {
            Log.Warn($"ModService: CurseForge search error: {ex.Message}");
        }
        ...
        ModProvider provider, string projectId, CancellationToken ct = default)
    {
        ...
        {
            Log.Warn($"ModService: failed to get project {projectId}: {ex.Message}");
            return null;
        }
    }
        ...
        IProgress<string>? progress = null, CancellationToken ct = default)
    {
        ...
        foreach (var f in files)
        {
            ...
            catch (Exception ex) { Log.Warn($"ModService: hash {Path.GetFileName(f)}: {ex.Message}"); }
        }
        ...
        try
        {
            ...
            if (!resp.IsSuccessStatusCode)
            {
                Log.Warn($"ModService: Modrinth version_files returned {(int)resp.StatusCode}");
                return updates;
            }
        ...
        {
            Log.Warn($"ModService: update check error: {ex.Message}");
            return updates;
        }
        ...
        foreach (var (hash, versionEl) in known)
        {
            ...
            {
                Log.Warn($"ModService: update for {Path.GetFileName(localPath)}: {ex.Message}");
            }
        }
        ...
        CancellationToken ct = default)
    {
        ...
        {
            Log.Warn($"ModService: failed to delete old version {update.CurrentName}: {ex.Message}");
        }
    ...
    public static List<LocalMod> GetLocalMods(string modsDir)
    {
        ...
        {
            Log.Warn($"ModService: failed to read mod list: {ex.Message}");
