...
public sealed class ServerPingService
{
    ...
    public async Task<ServerStatus> PingAsync(string address, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (packetId != 0x00)
                return ServerStatus.Offline($"Unexpected server response (id 0x{packetId:X2}).");
        ...
        {
            return ServerStatus.Offline("Response timeout exceeded.");
        }
        ...
        {
            return ServerStatus.Offline("Error: " + ex.Message);
        }
    }
    ...
    private static string DescribeSocketError(SocketException ex) => ex.SocketErrorCode switch
    {
        ...
        SocketError.ConnectionRefused => "Connection refused (server offline?).",
        SocketError.TimedOut => "Server not responding.",
        SocketError.NetworkUnreachable or SocketError.HostUnreachable => "Network unreachable.",
        _ => "Connection failed: " + ex.SocketErrorCode
    };
    ...
    {
        var a = (address ?? "").Trim();
        if (a.Length == 0) throw new ArgumentException("Empty server address.");
    ...
    private static (string host, int port)? QuerySrvViaNslookup(string host)
    {
        ...
        if (!p.WaitForExit(4000)) { try { p.Kill(); } catch (Exception ex) { Log.Warn(ex.Message); } return null; }
    ...
    private static string ReadString(Stream s)
    {
        ...
        if (len < 0 || len > 4 * 1024 * 1024) throw new InvalidDataException("Invalid string length.");
